...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_department'])) {
    ...
    if (mysqli_query($conn, $sql_delete_internship)) {
        ...
        echo "<script>alert('Error deleting related inter from internship : " . mysqli_error($conn) . "');</script>";
    }
}
        ...
        <?php if(isset($allDepartments) && !empty($allDepartments)): ?>
            <?php foreach ($allDepartments as $department): ?>
                <div class="department-box">
                    <div class="department-name" onclick="location.href='int_by_dep.php?department_id=<?php echo $department['id_depart']; ?>';">
                        <i class="fas fa-building"></i><?php echo " {$department['nam_depart']}"; ?>
                    </div>
                    <div class="department-options">
                        <div class="department-option1">
                            <button type="submit" class="department-option1" onclick="location.href='edit.php?id_depart=<?php echo $department['id_depart']; ?>';"><i class="fas fa-edit"></i>EDIT</button>
                        </div>
                        <form method="post" onsubmit="return confirm('Are you sure you want to delete this department?');">
                            ...
                            <button type="submit" class="department-option2"><i class="fas fa-trash"></i>DELETE</button>
                        </form>
                    </div>
                </div>
            <?php endforeach; ?>
